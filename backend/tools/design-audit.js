 ...
 * Usage: node design-audit.js [base-url]
 * Default base URL: http://localhost:5000
 *
 * Routes that need a project/recording ID (e.g. /project/1/tags) are
 * auto-discovered via the API if the server is running with data.
 */
...
// Dynamic routes are discovered at runtime — see discoverRoutes()
...
// CSS custom properties actually used by PINE
const CSS_VARS = [
  // Core palette
  '--bg', '--fg', '--card', '--muted', '--muted-fg',
  '--accent', '--accent-fg', '--accent-dim', '--accent-bd',
  '--border', '--secondary', '--sunken',
  // Destructive / warning
  '--destr', '--destr-dim', '--destr-bd',
  '--warn', '--warn-dim', '--warn-bd',
  // Typography & shape
...
const SKIP_IN_DIFF = new Set([
  ...
  'width', 'height', 'min-width', 'min-height', // vary by content, not a style issue
]);
...
async function auditPage(page, url) {
  // Page already navigated by the main loop — just collect styles
...
async function auditStates(page) {
  ...
  for (const btn of buttons.slice(0, 5)) {
    ...
    let hoverStyles = { ...defaultStyles };
    let focusStyles = { ...defaultStyles };
    try {
      await btn.hover({ timeout: 3000 });
      await page.waitForTimeout(150);
      hoverStyles = await btn.evaluate((el, props) => {
        const c = window.getComputedStyle(el);
        const s = {};
        for (const p of props) s[p] = c.getPropertyValue(p).trim();
        return s;
      ...
      await page.waitForTimeout(150);
      focusStyles = await btn.evaluate((el, props) => {
        const c = window.getComputedStyle(el);
        const s = {};
        for (const p of props) s[p] = c.getPropertyValue(p).trim();
        return s;
      }, STATE_PROPS);
    } catch (_) { /* element obscured or not interactable — skip hover/focus */ }
...
async function auditFocusRings(page) {
  ...
  for (const el of focusable.slice(0, 10)) {
    ...
    try {
      ...
      focusStyles = await el.evaluate((e, props) => {
        const c = window.getComputedStyle(e);
        const s = {};
        for (const p of props) s[p] = c.getPropertyValue(p).trim();
        return s;
      }, FOCUS_PROPS);
    } catch (_) { /* element not focusable or obscured */ }
...
async function discoverRoutes() {
  ...
  try {
    ...
    if (!resp.ok) return routes;
    const data = await resp.json();
    // API returns { active: [...], archived: [...] }
    const projects = [...(data.active || []), ...(data.archived || [])];
    // Pick the first project that has recordings
    for (const proj of projects) {
      routes.push(`/project/${proj.id}/tags`);
      const recResp = await fetch(`${BASE_URL}/api/projects/${proj.id}/recordings`);
      if (recResp.ok) {
      ...
      break; // one project is enough for the audit
    }
  } catch (e) {
    console.log(`  ⚠ Could not discover dynamic routes (${e.message}). Auditing static routes only.`);
  }
  return routes;
}
...
(async () => {
  ...
  for (const route of ROUTES) {
    ...
    try {
      ...
      await page.waitForTimeout(1500); // let JS/CSS fully render
