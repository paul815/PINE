...
class ProgressMapper:
    ...
    def __init__(self, total_duration, status, tick_secs=PROGRESS_TICK_SEC,
                 clock=time.monotonic, parallel=False):
        self._status = status
        self._clock = clock
    ...
    def _drift_pct(self):
        """Where the ticker thinks we are, between two real milestones."""
        start, end = self._bands[self._stage]
    ...
    def _emit(self, message, extra=None, from_tick=False):
        ...
        if from_tick:
            pct = self._advance(self._drift_pct())
        else:
            pct = self._advance(self._milestone_pct())
            self._anchor_pct = pct
            self._anchor_elapsed = self._elapsed()
        self._status(stage=self._stage, message=message or '',
                     percent=int(round(pct)), eta_secs=self._eta_secs(pct),
                     **(extra or {}))
...
def _expected_seconds(total_duration, parallel=False):
    ...
    expected = {
        ...
        'transcribing': secs * PROGRESS_RTF_TRANSCRIBE,
        'aligning': secs * PROGRESS_RTF_ALIGN,
        'diarizing': secs * PROGRESS_RTF_DIARIZE,
